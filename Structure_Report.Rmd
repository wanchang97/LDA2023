...
#### Response variable
From the `summary(trenal.long.noNA)`, we can read that the response variable is a continuous variable `HC`level from $(15,76)$ with Mean $38.24$. (Hematocrit is the percentage of red cells in your blood %)
...
```{r}
# Box plot by sex
ggplot(trenal.long.noNA,aes(x=as.factor(time),y=HC,fill=as.factor(male)))+ 
  geom_boxplot(position=position_dodge(1))
```
...
#fit.age = lm(HC ~ time +age,data=trenal.long.noNA)
#summ(fit.age)
