...
```{r}
anova(trenal.lm, trenal.lm2,  trenal.lm4, trenal.lm5, trenal.lm6)
```
...
```{r}
trenal_grouped<-groupedData(respons~time|id,trenal.long, labels=list(y="level of Haematocrit"),units=list(y="level of Haematocrit "))
...
bbdd <- cbind(bbdd,male=trenal.wide$male)
```
2nd step: 
```{r}

#Boxplot
meanbeta0bygroup<-tapply(bbdd$beta0,as.factor(bbdd$male),mean,na.rm=T)
meanbeta1bygroup<-tapply(bbdd$beta1,as.factor(bbdd$male),mean,na.rm=T)
